...
3. 絵付きの導入と形合わせでは、「あひるの あ」のように絵の名前と文字を結び付けます。音合わせの画面は正解語を出さず、「こえを きいて／おなじ もじを さがそう」と案内します。なぞり見本は枠から余白を空け、指やペンを動かしやすくしています。形・音・4つの書字を終えるたび、対象文字の小さな花と「できたね」が出ます。音声だけが正解根拠になる問題は、端末で音声が使えない時には自動で飛ばします。
