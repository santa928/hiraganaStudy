...
- 基本46文字: 絵付き導入、同じ形、太いなぞり、細いなぞり、お手本を見て書く、自分で書く、花の報酬
...
3. 絵付きの導入と形合わせでは、「あひるの あ」のように絵・文字・読み上げを一緒に結び付けます。同じ形を選んだ後は、音だけの問題を挟まず4段階の書字へ進みます。なぞり見本は枠から余白を空け、指やペンを動かしやすくしています。形と4つの書字を終えるたび、対象文字の小さな花と「できたね」が出ます。
4. 一文字が終わると庭に花が増えます。行を一通り終えた後だけ任意の「こえで おさらい」を出し、あ行は2択、以後は3択にします。「つぎへ」で飛ばせ、音声が使えない時は自動で飛ばします。
